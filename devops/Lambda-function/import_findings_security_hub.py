...
def process_message(event):
    ...
    if event['messageType'] == 'CodeScanReport':
        ...
        ### OWASP Zap SAST scanning report parsing
            ...
            alert_ct = event['report']['Report']['Sites']['Alerts']
            alert_count = len(alert_ct)
            for alertno in range(alert_count):
                risk_desc = event['report']['Report']['Sites']['Alerts']['AlertItem']['RiskDesc']
                ...
                if riskletters == 'Hig':
                    ...
                    normalized_severity = 90                                       
                # instances = len(event['report']['Report']['Sites']['Alerts']['AlertItem']['ItemCount'])
                finding_description = f"{alertno}-Vulerability:{event['report']['Report']['Sites']['Alerts']['AlertItem']['Alert']}"
